...
def load_and_prep_variants():
    ...
    # enrichment column already exists in data; ensure it's correct
...
def run_enrichment_analysis():
    ...
    # Step 3: For each epigenome, annotate variants with chromHMM state
    all_state_results = {}
    ...
    for eid, ename in EPIGENOMES.items():
        ...
        # Count states by enrichment group
        ...
        # Fisher's exact for each state
        state_results = []
        all_states = sorted(set(case_states.index) | set(ctrl_states.index))
        for state in all_states:
            a = case_states.get(state, 0)  # case in state
            b = n_case - a                  # case not in state
            c = ctrl_states.get(state, 0)   # ctrl in state
            d = n_ctrl - c                  # ctrl not in state
    ...
    # Step 4: Grouped analysis (regulatory vs non-regulatory)
    grouped_results = []
    for eid, ename in EPIGENOMES.items():
        col = f'state_{eid}'
    ...
    # Step 5: Cross-epigenome consensus
    # For each variant, count how many epigenomes it's in a regulatory state
    ...
    # Save variant-level annotations
...
def create_figures(results_df, grouped_df, consensus, df_mapped):
    ...
    # Figure 1: Heatmap of state proportions (case-enriched)
    fig, axes = plt.subplots(1, 2, figsize=(18, 8))

    # Case-enriched state distribution
    ...
    # Figure 2: Odds ratio forest plot for grouped states
    fig, ax = plt.subplots(figsize=(10, 8))

    # Focus on Active Regulatory across epigenomes
    ...
    # Figure 3: Cross-epigenome regulatory count distribution
    ...
    # Figure 4: Per-state odds ratio heatmap across epigenomes
    ...
    # Log2 OR for better visualization
