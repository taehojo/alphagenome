...
def load_snATACseq():
    ...
    with h5py.File(h5_path, 'r') as f:
        ...
        # Standard 10X format or custom format
        if 'matrix' in f:
            grp = f['matrix']
...
def parse_peak_coordinates(peak_ids):
    ...
    for pid in peak_ids:
        pid_str = str(pid)
        try:
            # Try formats: chr1:100-200, chr1-100-200, chr1_100_200
            if ':' in pid_str:
...
def build_peak_trees(peaks):
    ...
    for i, p in enumerate(peaks):
        ...
        trees[chrom][p['start']:p['end']] = i  # store peak index
...
def assign_cell_metadata(barcodes, meta):
    ...
    # Find barcode column in meta (prefer exact 'Barcode' match)
    barcode_col = None
    ...
    # Find cell type column (prefer 'Cell.Type' or 'cell_type' over generic 'cluster')
    celltype_col = None
    for col in meta.columns:
...
def compute_pseudobulk_accessibility(matrix, cell_types, variant_peak_map):
    ...
    for ct in ['Neuron', 'Microglia', 'Astrocyte', 'Other']:
        ...
        # Sum across cells of this type for relevant peaks
        ct_matrix = matrix_csr[:, ct_mask]

        for pidx in peak_indices:
            if pidx >= matrix_csr.shape[0]:
                continue
            # Mean accessibility across cells
            peak_access = ct_matrix[pidx, :].mean()

            # Get variant info for this peak
...
def analysis_open_chromatin_enrichment(variant_df, variant_peak_map):
    ...
    # Which variants overlap any peak?
...
def figure_B2_celltype_accessibility(access_df):
    ...
    # Panel a: By accessibility cell type
    ax = axes[0]
    ...
    # Panel b: Case-enriched vs Control-enriched
    ax = axes[1]
...
def figure_B4_gene_accessibility_heatmap(access_df):
    ...
    # Gene x cell-type mean accessibility
    ...
    # Color gene labels by paper cell type
